...
**Kustomize:** if the workload's manifest is generated by Kustomize, point at the directory instead of a file with `--kustomize-dir`. kubeloop traces the rendered workload (including a `namePrefix`/`nameSuffix` overlay) back to the **source** file and patches that — never a rendered file your GitOps controller would regenerate. Overlays that could rename a resource (`patches`, generators, `replacements`) are refused with a clear message rather than mis-mapped.

```bash
kubeloop pr --from-file scan.json --kustomize-dir overlays/prod \
  --workload prod-checkout-api --container app --open --dry-run
```
