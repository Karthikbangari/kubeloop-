...
- **REAL BUG — kubeloop would clobber git internals.** `--manifest .git/hooks/pre-commit` is lexically inside the repo, its parent resolves inside the repo, and its leaf is a genuine regular file — so #82's and #83's guards both pass. `openpr` overwrote it and returned **`err = nil`**. `.git/config` likewise. Severity is real: the patch is written *before* `git commit`, so a clobbered `pre-commit` hook persists and executes on the next commit anyone makes in that checkout; `.git/config` controls where `push` sends data.
  - **Fix:** `patchTarget` refuses any path with a `.git` segment, compared **case-insensitively** (macOS and Windows filesystems, and git itself, treat `.GIT` as the git directory). Refused before any branch/commit/push/GitHub call; git internals verified byte-for-byte unchanged.
  - **Not over-broad:** the guard matches a path *segment*, not a substring — `.gitignore`, `deploy/.gitkeep`, and `gitops/app.yaml` are all still allowed. Pinned by `TestOpen_AllowsGitLikeFilenames` so a future tightening can't silently break real repos.
- **Guard added — detached HEAD.** `git rev-parse --abbrev-ref HEAD` reports the literal `"HEAD"` when detached. Opening a PR onto a branch named `HEAD` would 422 *after* the branch had already been pushed. Now refused up front with "pass --base".
- **Checked, no bug:** the patched write does **not** widen file permissions — `os.WriteFile` preserves an existing file's mode, so a `0600` manifest stays `0600`. Pinned by `TestOpen_PreservesFileMode` rather than left to chance.
- **The GitHub POST is now partially validated against the real api.github.com** (previously httptest-only), without a credential and without creating anything:
  - `ghclient.CreatePR` with a deliberately invalid token → real GitHub **401** → mapped to `github rejected the token (401): check GITHUB_TOKEN is valid and unexpired`, and the token did **not** leak into the error. This exercises DNS, TLS, the POST, the `Authorization` header format (GitHub parsed it and rejected the credential), the `Accept`/`X-GitHub-Api-Version` headers, our status→message mapping, and `scrub()`.
  - 401 fires before repo lookup, so the URL path was checked separately with a read-only GET. **This confirmed #75's trailing-dash bug is real, against the live API:** `/repos/Karthikbangari/kubeloop-` → **200**, while `/repos/Karthikbangari/kubeloop` (what a cutset `.git` trim yields) → **404**. `ParseOrigin`'s output is what GitHub actually accepts.
...
- **Feature:** `kubeloop pr --open [--repo-dir DIR] [--base BRANCH] [--dry-run]`. `--out` (write the patched manifest locally) and `--open` (branch, commit, push, open a PR) are mutually exclusive — requiring exactly one keeps the command's side effects unambiguous. `--dry-run` without `--open` is an error rather than a silent no-op.
- **Bug found by wiring it (the guard was right, the ergonomics were wrong):** the first live dry-run failed with *"manifest path … must be relative to the repository"*. `openpr.safeJoin` correctly refuses absolute paths, but users naturally pass `--manifest /abs/path/deploy.yaml`. The fix belongs in the CLI, which is the layer that knows `--repo-dir`: `repoRelative` now relativizes the manifest against the checkout and refuses one outside it **by name** (`manifest X is not inside --repo-dir Y`) rather than deferring to openpr's generic escape message. The security guard is untouched; only the path handed to it is now correct.
- **Token handling:** `--open` without `GITHUB_TOKEN`/`GH_TOKEN` fails *before* any git mutation, and the message points at `--dry-run`. The keychain PAT that `git push` uses was deliberately never read.
- **Verified live (real git, real filesystem, no GitHub):** in a throwaway checkout with a GitHub remote, `pr --open --dry-run` printed the patch target, the content-keyed branch `kubeloop/rightsize-shop-checkout-api-c23f47d`, and the PR title — then left the repo on `main`, with no new branch and a clean working tree. `--open` without a token exited 1 with an actionable message.
...
- **REAL BUG FOUND AND FIXED — `HistoryDays` measured the longest-lived *pod*, not the workload.** It was `max(count_over_time(sum by (pod) (…)[30d:1d]))`. Pods are replaced on every deploy, so a workload that has run for a year but ships daily has no pod older than a day → reports HistoryDays≈1 → **silently excluded by safety as "<7d of history."** Teams that deploy frequently would watch their entire cluster vanish from the report, with no error and no explanation. Fix: aggregate the `pod` label away *before* counting — `count_over_time(sum(…)[30d:1d])` — which counts the days on which *any* pod of the workload existed.
  - **Proved empirically, not by argument.** Churned a deployment's pod every 90s for 6 minutes (5 distinct pods), then measured over an 8m window that no single pod spanned: the old form returned **6**, the corrected form **16** — the old form saw 38% of the workload's true age. Per-pod counts were 3/5/5/5/6.
  - CPU and memory correctly *stay* per-pod-then-max (`sum by (pod)` → `max`): a request is sized per pod, so the busiest pod is what matters. Only history is a workload-level property. Regression tests pin both.
- **Assumptions confirmed on real data:** `container_cpu_usage_seconds_total` and `container_memory_working_set_bytes` exist with `namespace`/`pod`/`container` labels. `container!=""` is load-bearing — it dropped 10 of 15 series in the namespace (the per-pod cgroup rollups), leaving exactly the real pods.
- **The over-match guard, proved against real pod names** (not just a regex unit test): `pod=~"checkout-api-.*"` matched **three** pods including `checkout-api-v2-854984d696-sgx9s` — a different service — while the kind-aware `pod=~"checkout-api-[a-z0-9]+-[a-z0-9]+"` matched exactly checkout-api's two. Sizing `checkout-api` with the naive regex would have summed another service's load into its numbers.
- **Assumption refuted (harmlessly):** `container!="POD"` matches nothing on modern cadvisor, which no longer emits the pause container under that name. Kept as a guard for older clusters (k8s <1.16); documented as a no-op today rather than left to imply it does something.
- **Prometheus subquery alignment (learned, documented):** subquery steps align to absolute multiples of the step, so a workload younger than one step lands between evaluation points and the query returns *no data*. This fails safe — `clustersource` maps no-data to zero, and safety excludes rather than sizes. A minutes-old workload yields an empty `[30d:1d]` result because no midnight boundary has passed.
- **End-to-end result:** real `kubectl` → real Prometheus → `promusage` → `clustersource` → `scan`. `busy` measured **p95 = 0.99999 cores → 1000m**; `churn` p95 1000m / 3Mi; the pause-image workloads correctly measured 0 CPU and were excluded as "no measured CPU usage"; everything was excluded as **"only 0d usage history (<7d) — not enough signal."** No ranked dollar figure was produced, and none was faked: the cluster is 40 minutes old and the tool refuses to size on thin data. **The guardrail working on a live cluster is the result.**
