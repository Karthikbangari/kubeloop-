...
build: ## build the kubeloop binary into bin/
	go build -o $(BIN) ./cmd/kubeloop
...
ci: vet test ## what CI runs: vet + tests
...
release-snapshot: ## build release artifacts locally with goreleaser (no publish)
...
.PHONY: build test vet ci clean release-check release-snapshot dev-up workloads prom dev-down help
